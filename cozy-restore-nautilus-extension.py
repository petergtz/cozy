	...
	def get_toolbar_items(self, window, path):
		...
		try:
			...
			if not self.is_in_restore_mode:
				item = nautilus.MenuItem(name="NautilusPython::restoremode", icon='document - open - recent', label="Go to Restore Mode", tip="Switches Nautilus into Bakcupmode to discover older versions of file and folders")
				item.connect("activate", self.go_to_restore_mode, path, window)
				items.append(item)
			else:
				item = nautilus.MenuItem(name="NautilusPython::closerestorebackupmode", label="Close Restore Mode", tip="", icon='exit')
